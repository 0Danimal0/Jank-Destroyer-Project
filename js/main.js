...
// Store the items variable that looks at the classes in the global scope.
...
function updatePositions() {
  ...
  for (var i = 0; i < 7; i++) {
    scrollCalcArray.push(Math.sin((scrollPos / 1250)+i));
  };

  for (var i = 0; i < items.length; i++) {
    phase = scrollCalcArray[i % 5];
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //use requestAnimationFrame to load positions of all pizzas when page loads.
  window.requestAnimationFrame(updatePositions);
});
